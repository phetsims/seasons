...
define( function( require ) {
  ...
  return inherit( Node, LightNode, {
    ...
    setLightRect: function() {
      this.beamNode.shape = new Shape().moveTo( this.rightLayout, this.beamBottom ).lineTo( -1000, this.beamBottom ).lineTo( -1000, this.beamTop ).lineTo( this.rightLayout, this.beamTop ).close();
    },
    ...
    setLightProjection: function( tailX, tipX, centerX, centerY, radiusX, radiusY, rotation, clipBottom, clipTop, panelType ) {
      ...
      this.beamNode.shape = ( panelType === 'solar' && rotation !== Math.PI ) ?
                            new Shape().moveTo( this.rightLayout, this.beamBottom ).lineTo( -1000, this.beamBottom ).lineTo( -1000, clipBottom ).lineTo( centerX + radiusX, clipBottom ).// the difficult calculation to make is which angles to draw the elliptical arc between. The angles need to align
                            // with the clipTop and clipBottom points. This is just an approximation right now
                            ellipticalArc( centerX, centerY, radiusX, radiusY, rotation, -Math.PI / 2 + offset * 0.9, Math.PI / 2 - offset * 1.05, true ).lineTo( -1000, clipTop ).lineTo( -1000, this.beamTop ).lineTo( this.rightLayout, this.beamTop ).close() :
                            new Shape().moveTo( this.rightLayout, this.beamBottom ).lineTo( tailX, this.beamBottom ).ellipticalArc( centerX, centerY, radiusX, radiusY, rotation, 3 * Math.PI / 2, 3 * Math.PI / 2 + Math.PI, true ).lineTo( this.rightLayout, this.beamTop ).close();
    }
  } );
} );